@testset "Assembler" begin
    @testset "linear-single-scalar-fe" begin
        ...
        #--- Mesh 3
        # Issue #249
        # Build a face of length "1" between a square and a triangle.
        # We integrate `side_n(v)+side_n(p)` so the result should be `1` for each
        # shape function (dof) lying on the face.
        nodes = Node.([[0.0, 0.0], [1.0, 0.0], [1.0, 1.0], [0.0, 1.0], [2.0, 0.0]])
        celltypes = [Bcube.Quad4_t(), Bcube.Tri3_t()]
