...
function assemble_bilinear_elemental!(
    values,
    cache,
    U::TrialFESpace,
    V::TestFESpace,
    ...
    f(u, v) = get_function(get_integrand(a(u, v)))
    _assemble_bilinear_elemental!(values, cache, U, V, f, measure)
end

function _assemble_bilinear_elemental!(
    values,
    cache,
    U::TrialFESpace,
    V::TestFESpace,
    ...
    quadrature = get_quadrature(measure)
    I, J, E, Iloc, Jloc = cache

    AK.foreachindex(values) do k
        elementInfo, iloc, jloc = E[k], Iloc[k], Jloc[k]
        cshape = shape(celltype(elementInfo))
        λu = get_shape_functions(U, cshape)[jloc]
        λv = get_shape_functions(V, cshape)[iloc]
        _f = f(λu, λv)
        g = materialize(_f, elementInfo)
        values[k] = integrate_on_ref_element(g, elementInfo, quadrature)
    end

    return nothing
end

function build_cache_assemble_bilinear_elemental(U, V, measure)
    # Alias
    quadrature = get_quadrature(measure)
    domain = get_domain(measure)

    # Loop over cells
    A = map(DomainIterator(domain)) do elementInfo
        icell = cellindex(elementInfo)
        nU = Val(get_ndofs(U, shape(celltype(elementInfo))))
        nV = Val(get_ndofs(V, shape(celltype(elementInfo))))
        Udofs = get_dofs(U, icell, nU) # columns correspond to the TrialFunction
        Vdofs = get_dofs(V, icell, nV) # lines correspond to the TestFunction
        rows, cols = _cartesian_product(Vdofs, Udofs)
        loc_i, loc_j = _cartesian_product(ntuple(identity, nV), ntuple(identity, nU))
        rows, cols, fill(elementInfo, length(rows)), loc_i, loc_j
    end

    return ntuple(i -> rawcat(getindex.(A, i)), length(A[1]))
end